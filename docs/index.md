...
hero:
  ...
  tagline: symtether checks the symbol references in your markdown against the code itself, and fails CI when they break. There is no config and no lockfile. Plain markdown links are the only state.
  actions:
    - theme: brand
      text: Get started
      link: /guide
